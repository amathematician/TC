...
public:
	vector<string> decode(string message) {
    	...
    	str1.resize(n);
    	str2.resize(n);
    	// construct str1 by decoding message
    	str1[0] = '0';
    	str1[1] = ((message[0] - '0') - (str1[0] - '0')) + '0';
    	if (str1[1] >= '2') {
        	//cout << "str1[1] >= 2: pushing NONE" << endl;
        	res.push_back("NONE");
    	} else {
        	for (int i = 2; i <= n-1; i++) {
            	str1[i] = ((message[i-1] - '0') - (str1[i-1] - '0') - (str1[i-2] - '0')) + '0';
            	if(str1[i] >= '2') {
                	//cout << "str1[i] >= 2: pushing NONE" << endl;
                	res.push_back("NONE");
                	break;
            	}
        	}
    	...
    	if (res.size() != 1) {
        	if (message[n-1] != ((str1[n-2] - '0') + (str1[n-1] - '0')) + '0') {
            	//cout << "message[n-1] does not match str1[n-1]: pushing NONE" << endl;
            	res.push_back("NONE");
        	} else {
            	res.push_back(str1);
        	}
    	}
    	// construct str2 by decoding message
    	str2[0] = '1';
    	str2[1] = ((message[0] - '0') - (str2[0] - '0')) + '0';
    	if (str2[1] >= '2') {
        	//cout << "str2[1] >= 2: pushing NONE" << endl;
        	res.push_back("NONE");
    	} else {
       		for (int i = 2; i <= n-1; i++) {
            	str2[i] = ((message[i-1] - '0') - (str2[i-1] - '0') - (str2[i-2] - '0')) + '0';
            	if (str2[i] >= '2') {
               		//cout << "str2[i] >= 2: pushing NONE" << endl;
                	res.push_back("NONE");
                	break;
            	}
        	}
    	...
    	if (res.size() != 2) {
        	if (message[n-1] != ((str2[n-2] - '0') + (str2[n-1] - '0')) + '0') {
            	//cout << "message[n-1] does not match str2: pushing NONE" << endl;
            	res.push_back("NONE");
        	} else {
            	res.push_back(str2);
        	}
